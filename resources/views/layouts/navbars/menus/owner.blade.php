<ul class="navbar-nav">
	...
	<li class="nav-item">
        <a class="nav-link" href="{{ route('webs.index',  auth()->user()->restorant->id) }}">
            <i class="ni ni-world-2"></i> {{ __('Web Site') }}
        </a>
    </li>	
    ...
    @if (config('app.isqrsaas') && (!config('settings.qrsaas_disable_odering') || config('settings.enable_guest_log')))
        @if(!config('settings.is_whatsapp_ordering_mode') || in_array("poscloud", config('global.modules',[])))
            <li class="nav-item">
    ...
    @if (config('app.isqrsaas')&&(config('settings.is_whatsapp_ordering_mode')||in_array("poscloud", config('global.modules',[])) ))
        <li class="nav-item">
